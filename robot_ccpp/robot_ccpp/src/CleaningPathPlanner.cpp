...
: Node("path_planning_node") {
    ...
    grid_pub_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("covered_grid", rclcpp::QoS(rclcpp::KeepLast(10))) ;
...
void CleaningPathPlanning::GetBorderTrackingPathInCV(vector<cv::Point2i> &resultVec)
{
    ...
    for (r = 0; r < srcMap_.rows; r++)
    {
        ...
        {
            //RCLCPP_INFO(this->get_logger(), "The value at row %d, col %d is %d",r,c,srcMap_.at<uchar>(r,c));
            if (srcMap_.at<uchar>(r, c) == nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
            {
                RCLCPP_INFO(this->get_logger(), "entered once!");
                if (srcMap_.at<uchar>(r, c - 1) != srcMap_.at<uchar>(r, c + 1) && (srcMap_.at<uchar>(r, c - 1) == nav2_costmap_2d::FREE_SPACE || srcMap_.at<uchar>(r, c + 1) == nav2_costmap_2d::FREE_SPACE))
                {
                    temppoint.x = c;
                    temppoint.y = r;
                    borderPointsIndexVec.push_back(temppoint);
                }
                else if (srcMap_.at<uchar>(r - 1, c) != srcMap_.at<uchar>(r + 1, c) && (srcMap_.at<uchar>(r - 1, c) == nav2_costmap_2d::FREE_SPACE || srcMap_.at<uchar>(r + 1, c) == nav2_costmap_2d::FREE_SPACE))
                {
                    temppoint.x = c;
                    temppoint.y = r;
                    borderPointsIndexVec.push_back(temppoint);
...
void CleaningPathPlanning::mainPlanningLoop()
{
    ...
    if (!isok)
    {
        RCLCPP_INFO(this->get_logger(), "Failed to get robot location! Please check where goes wrong!");
        return;
    }
